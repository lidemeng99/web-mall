...
<script>
import { fetchCancelApproveList, updateState } from '@/api/product'
export default {
  data() {
    return {
      products: null
  ...
  created() {
    fetchCancelApproveList(res => {
      this.products = res
    })
  },
  methods: {
    doApprove(item, index) {
      const _data = { type: 'CANCEL_APPROVE', operator: this.$store.getters.name, item: item }
